      ...
      child: FutureBuilder<List<StakeRow>>(
        ...
        builder: (BuildContext context, AsyncSnapshot<List<StakeRow>> snapshot) {
          if (snapshot.connectionState == ConnectionState.done) {
            ...
            final stakeColor = allStakes.isEmpty ? Theme.of(context).primaryTextTheme.button.backgroundColor : Theme.of(context).accentTextTheme.caption.decorationColor;
            var totalAmountStaked = 0;
            for (final stake in allStakes) {
              totalAmountStaked += stake.amount;
            }
            final stakePercentage = allStakes.isEmpty ? 1.0 : min(belDexAmountToDouble(totalAmountStaked) / 10000, 1.0);
            return ListView(
              shrinkWrap: true,
              ...
              child: Container(
                ...
                child: Center(
                  child: CircularProgressIndicator(
                    ...
                    backgroundColor: Theme.of(context).accentTextTheme.caption.decorationColor,
                  ),
                )
              ),
            );
